...
def segment_image(image: Union[str, np.ndarray], 
                  ...
                  *, cellpose_kwargs: Dict[str, Union[int, float]] = {"diameter": 50, "min_size": 10},
                  nuclear_channel=2) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    """
    img = tifffile.imread(image) if isinstance(image, str) else image
    if not isinstance(img, np.ndarray):
        raise TypeError("Invalid type for 'image'. Must be either str or np.ndarray.")
    
    if isinstance(model, str):
        if os.path.exists(model):
            model = models.CellposeModel(pretrained_model=model, gpu=torch.cuda.is_available())
        elif model in AVAIL_MODELS:
            model = models.CellposeModel(pretrained_model=f"{CELLPOSE_PATH}/{model}", gpu=torch.cuda.is_available())
        else:
    ...
    masks, _, _ = model.eval(img[..., nuclear_channel], diameter=model.diameter, min_size=model.min_size)
...
def extract_patches(image_path_or_array: Union[str, np.ndarray], 
                    model_path_or_type: Union[str, models.Cellpose, models.CellposeModel], 
                    patch_size: int = 50, 
                    cellpose_kwargs: Dict[str, Union[int, float]] = {"diameter": 50, "min_size": 10},
    ...
    Parameters:
        image_path_or_array: Union[str, np.ndarray] - Either a path to an image file or a numpy array representing the image.
        model_path_or_type: Union[str, models.Cellpose, models.CellposeModel] - Either a path to a pre-trained model, or a Cellpose model instance.
    ...
    masks, original_image = segment_image(image_path_or_array, model_path_or_type, cellpose_kwargs=cellpose_kwargs, nuclear_channel=nuclear_channel)
